...
| `rule.repeat_alert_answered` | the owner answered a rule that had adjusted the same night 3 times or more: one event per answer, however many nights it settled | `POST /api/rules/alerts/[alertId]` | `rule_id`, `choice` (`keep_adjusting` or `stop`), `nights`, `all_nights`, `simulation` |
