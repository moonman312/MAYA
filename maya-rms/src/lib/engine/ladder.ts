 ...
 * affected_room_type), tracks is_active state and emits transition events.
 *
 * The pass runs against an in-memory copy of ladder_rule_state (preloaded
 * for the whole horizon) and buffers its writes; flushLadderWrites lands
 * them in two bulk statements instead of one round-trip per triple. The
 * in-memory map is what makes the duplicate-affected edge behave: a rule
 * listing the same room type twice sees its own first visit's activation on
 * the second visit, exactly as the old read-your-writes queries did.
 */
