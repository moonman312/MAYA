-- MAYA Command Center v3 Migration — service_role bypass fix
-- ============================================================================
-- BUGFIX: the platform_* RPCs in v2 checked `is_platform_admin() OR
-- can_manage_hotel()` but did not treat service_role callers as pre-authorized.
-- When the Next.js /admin routes invoke these via the service-role client,
-- auth.uid() returns NULL, both checks return false, and the RPC errors with
-- "Not authorized to invite users to hotel <uuid>".
--
